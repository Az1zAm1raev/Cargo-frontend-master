            ...
            <div class="card mt-5" v-if="flag">
                <div class="card-body">
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="submit" class="btn btn-success" value="Confirm" v-if="checkedOrder.status == 'TAKEN'" @click="confirm">
                        <input type="submit" class="btn btn-primary" value="Decline" v-if="checkedOrder.status == 'TAKEN'" @click="decline">
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        orderClicked(item) {
            ...
            if (this.checkedOrder.status == "SHIPPED") {
                var lonlat = { lon: this.checkedOrder.currentLocation.longitude, lat: this.checkedOrder.currentLocation.latitude };
                var iconFav = new ALKMaps.Icon(
                ALKMaps.IMAGE.TRUCK_BLUE,
                new ALKMaps.Size(30, 30)
                );
                this.marker = new ALKMaps.Marker(
                lonlat,
                iconFav.clone(),
                "Deliver",
        ...
        confirm() {
            axios
            .get('http://localhost:8080/shippers/order/' + this.checkedOrder.id + '/confirm',
            {
                headers:{
                    Authorization: this.$store.getters.getToken,
                }
        ...
        decline() {
            axios
            .get('http://localhost:8080/shippers/order/' + this.checkedOrder.id + '/decline',
            {
                headers:{
                    Authorization: this.$store.getters.getToken,
                }
    ...
    mounted() {
        ...
        this.map.addLayers([base, this.markerLayer]);
    }
}
</script>
